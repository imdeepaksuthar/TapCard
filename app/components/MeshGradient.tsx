...
// Uses only inline styles so isDark prop changes trigger live re-renders.
export default function MeshGradient({ color, isDark = true, intensity = 0.55, className = '' }: Props) {
  ...
  return (
    ...
    >
      {/* Inject keyframe + orb base styles once (not dynamic, so safe in a global style tag) */}
      <style>{`
        @keyframes meshFloat {
        ...
        @media (prefers-reduced-motion: reduce) {
          .mesh-orb-animated { animation: none !important; }
        }
      `}</style>
