        ...
        {isOpen && (
          ...
          >
            <div className="p-4 border-b border-[var(--d-border)] flex items-center justify-between bg-[var(--d-surface-2)]">
              ...
              {unreadCount > 0 && (
                <button
                  ...
                  className="text-xs text-[var(--d-accent)] hover:opacity-80"
                >
                  Mark all read
                </button>
              )}
            ...
            <div className="max-h-[400px] overflow-y-auto">
              {unreadCount === 0 ? (
                <div className="p-8 text-center text-[var(--d-text-muted)]">
                  ...
                  {notifications.map((notification) => {
                    ...
                    return (
                      <div 
                        ...
                        className="p-4 hover:bg-[var(--d-hover)] transition-colors cursor-pointer group flex items-start gap-3"
                      >
                        ...
                        <div>
                          <p className="text-sm text-[var(--d-text-muted)]">
                            <span className="font-semibold text-[var(--d-text)]">{notification.data.name}</span>
                            {isOrder ? ' placed a new order on ' : ' left a new inquiry on '}
                            <span className="font-medium">{notification.data.card_name}</span>.
                          </p>
                          <p className="text-xs text-[var(--d-text-faint)] mt-1">
            ...
            {unreadCount > 0 && (
              <div className="p-3 border-t border-[var(--d-border)] bg-[var(--d-surface-2)] text-center">
                <button
                  onClick={() => { setIsOpen(false); router.push('/dashboard/leads'); }}
                  className="text-sm font-medium text-[var(--d-text-muted)] hover:text-[var(--d-text)] transition-colors"
                >
                  View all leads
                </button>
              </div>
