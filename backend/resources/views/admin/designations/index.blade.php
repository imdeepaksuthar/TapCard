<x-admin.layout>
    ...
    <div x-data="{ showDeleteModal: false, deleteUrl: '' }">
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="{{ route('admin.designations.create') }}" class="inline-flex items-center justify-center gap-2.5 rounded-md bg-primary py-2 px-6 text-center font-medium text-white hover:bg-opacity-90 transition-all duration-300 shadow-sm">
                <span>
        ...
        <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark p-4 sm:p-6 xl:p-7.5">
                ...
                <table id="designationsTable" class="w-full table-auto">
                    <thead>
                        <tr class="bg-gray-2 text-left dark:bg-meta-4">
                            <th class="py-4 px-4 font-medium text-black dark:text-white xl:px-7.5">Name</th>
                        ...
                        @foreach($designations as $designation)
                            <tr>
                                ...
                                <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    <span class="inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium {{ $designation->status === 'active' ? 'bg-success text-success' : 'bg-danger text-danger' }}">
                                        {{ ucfirst($designation->status) }}
                                    </span>
                                </td>
                                <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    <div class="flex items-center space-x-3.5">
                                        <a href="{{ route('admin.designations.edit', $designation->id) }}" class="hover:text-primary transition-colors">
                                            <svg class="fill-current w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                                <path d="M5 19h1.414l9.707-9.707-1.414-1.414L5 17.586V19zm15.707-11.293a1 1 0 000-1.414l-2.293-2.293a1 1 0 00-1.414 0l-1.586 1.586 3.707 3.707 1.586-1.586zM3 21v-4.243L16.435 3.322a3 3 0 014.243 0l2.293 2.293a3 3 0 010 4.243L9.243 21H3z" />
                                            </svg>
                                        </a>
                                        <button type="button" @click="showDeleteModal = true; deleteUrl = '{{ route('admin.designations.destroy', $designation->id) }}'" class="hover:text-[#EB5757] transition-colors text-gray-500">
                                            <svg class="fill-current w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                                <path d="M9 3h6v2H9V3zm10 4H5v14a2 2 0 002 2h10a2 2 0 002-2V7zm-8 12H9v-8h2v8zm4 0h-2v-8h2v8zm1-14V3a2 2 0 00-2-2H9a2 2 0 00-2 2v2H3v2h18V5h-5z" />
                                            </svg>
                                        </button>
                                    </div>
                                </td>
        ...
        <div x-show="showDeleteModal" 
            ...
            <div @click.outside="showDeleteModal = false"
                ...
                <div class="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-[#EB5757]/20 text-[#EB5757]">
                ...
                <p class="mb-6 text-gray-500 dark:text-gray-400">Are you sure you want to delete this designation? This action cannot be undone.</p>
                
                <div class="flex flex-col gap-3 sm:flex-row sm:justify-center">
                    <button @click="showDeleteModal = false" class="block rounded-md border border-stroke px-6 py-2.5 font-medium text-black transition hover:bg-gray-100 dark:border-strokedark dark:text-white dark:hover:bg-meta-4">
                        Cancel
                    </button>
                    <form :action="deleteUrl" method="POST" class="inline-block">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="block w-full rounded-md bg-[#EB5757] px-6 py-2.5 font-medium text-white transition hover:bg-opacity-90 sm:w-auto">
                            Yes, Delete It
                        </button>
                    </form>
                </div>
