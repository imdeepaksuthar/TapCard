<x-admin.layout>
    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm font-medium text-gray-500 dark:text-gray-400">Update your existing ad campaign.</p>
        </div>
    ...
    <!-- Form Section -->
    <form action="{{ route('admin.advertisings.update', $advertising) }}" method="POST" enctype="multipart/form-data" class="w-full" novalidate>
        @csrf
        @method('PUT')
                ...
                <div class="rounded-2xl border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark w-full">
                    ...
                    <div class="p-6 space-y-6 w-full">
                        ...
                        <div class="w-full">
                            ...
                            <input type="text" name="title" value="{{ old('title', $advertising->title) }}" placeholder="e.g., Summer Sale 2026"
                ...
                <div class="rounded-2xl border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark w-full">
                    ...
                    <div class="p-6 w-full">
                        @if($advertising->image_path)
                            <div class="mb-4">
                                <p class="text-sm font-medium text-black dark:text-white mb-2">Current Image:</p>
                                <img src="{{ asset('storage/' . $advertising->image_path) }}" alt="Current Banner"
                                    class="h-32 object-cover rounded shadow-sm border border-stroke dark:border-strokedark">
                            </div>
                        @endif

                        <label class="mb-2.5 block text-sm font-medium text-black dark:text-white">
                            Upload Advertisement Image (Leave empty to keep current)
                        </label>
                        <div class="relative flex flex-col items-center justify-center w-full h-56 border-2 border-dashed border-primary/30 rounded-2xl bg-gray-50 hover:bg-gray-100 transition-colors dark:border-strokedark dark:bg-meta-4 dark:hover:bg-meta-3 cursor-pointer overflow-hidden group box-border">
                            ...
                            <img id="image-preview" src="#" alt="Preview" class="hidden absolute inset-0 w-full h-full object-cover z-10 pointer-events-none" />
                        ...
                        @error('image')
                                <p class="text-red-500 text-sm mt-2 font-medium relative z-30">{{ $message }}</p>
                            @enderror
                    </div>
                </div>
                    ...
                    <div class="p-6 space-y-5 w-full">
                        ...
                        <div class="w-full">
                            ...
                            <select name="position"
                                ...
                                required>
                                <option value="" disabled>Select Ad Position</option>
                                <option value="top" {{ old('position', $advertising->position) == 'top' ? 'selected' : '' }}>Top (Leaderboard · 728×90)</option>
                                <option value="right" {{ old('position', $advertising->position) == 'right' ? 'selected' : '' }}>Right (Skyscraper · 300×600)</option>
                                <option value="bottom" {{ old('position', $advertising->position) == 'bottom' ? 'selected' : '' }}>Bottom (Medium Rectangle)</option>
                            </select>
                            @error('position')
                                <p class="text-red-500 text-sm mt-2 font-medium">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="w-full">
                            ...
                            <select name="status"
                                ...
                                <option value="active" {{ old('status', $advertising->status) == 'active' ? 'selected' : '' }}>Active (Visible)</option>
                                <option value="inactive" {{ old('status', $advertising->status) == 'inactive' ? 'selected' : '' }}>Inactive (Hidden)</option>
                            </select>
                            @error('status')
                                <p class="text-red-500 text-sm mt-2 font-medium">{{ $message }}</p>
                            @enderror
                ...
                <div class="rounded-2xl border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark w-full">
                    ...
                    <div class="p-6 space-y-5 w-full">
                        ...
                        <div class="w-full">
                            ...
                            <input type="date" name="start_date" value="{{ old('start_date', $advertising->start_date ? $advertising->start_date->format('Y-m-d') : '') }}" required
                        ...
                        <div class="w-full">
                            ...
                            <input type="date" name="end_date" value="{{ old('end_date', $advertising->end_date ? $advertising->end_date->format('Y-m-d') : '') }}" required
                                class="w-full block box-border rounded-xl border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input" />
                            @error('end_date')
                                <p class="text-red-500 text-sm mt-1 font-medium">{{ $message }}</p>
                            @enderror
